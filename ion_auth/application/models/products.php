...
class Products extends CI_Model
{
    ...
    public function product($id=1){
        ...
        $this->db->select('*') ->from('offer_time_validity') ->where('prod_meat_view', $id);
        $meat  =  $this->db->get()->result() ;
        echo '<pre>'; print_r($meat);

        $this->db->select('*') ->from('prod_options_price_view') ->where('prod_meat_view', $id);
