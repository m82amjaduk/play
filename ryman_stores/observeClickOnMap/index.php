<!--http://learning7.lc/ryman_stores/observeClickOnMap/-->
<html>
<head>
...
//         $collect = '<div onclick="myFunction(amzad)"> Collect From This Store </div>';
        ...
        google.maps.event.addListener(<?=$name?>,'click',function() {
            ...
            infowindow.setContent('<img src="http://www.retail-week.com/pictures/636xAny/8/6/4/1259864_Ryman_2.jpg" width="300px"/> <br />this store is ' + '<?=$name?>' + "<?=$collect?>" );
...
<?
foreach ( $res as $row){ $name = $row['unique_name']?>
<?=$name?> <button type="button" onclick="showMarker(<?=$name?>)" class="select" name="<?=$name?> selected" >Show On Map</button> <br>
<?}?>
